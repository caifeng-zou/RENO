...
class GNOBlock(nn.Module):
    ...
    def forward(self, y, x, f_y=None, reduction='sum'):
        ...
        if self.neighbors_dict is None:  # cache the neighbors dict, avoid redundant neighbor search for fixed query points
            self.neighbors_dict = self.neighbor_search(data=y, queries=x, radius=self.radius)
